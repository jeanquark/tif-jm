	...
	export default {
		...
		computed: {
			...
			allEvents() {
				...
				return (
					this.$store.getters['events/loadedEvents']
			...
			events() {
				// const today = moment().format('YYYY-MM-DD')
				return this.$store.getters['events/loadedEvents'].filter(event => event.date === this.date.format('YYYY-MM-DD')).sort((a, b) => a.timestamp - b.timestamp)
			},
			userEvents() {
				const today = moment().format('YYYY-MM-DD')
				const userTeamsIds = this.userTeamsIds
		...
		methods: {
			...
			async eventsByDate(setDayNumber) {
				...
				const events = this.$store.getters['events/loadedEvents'].filter(event => event.date === this.date.format('YYYY-MM-DD'))
				if (events.length < 1) {
