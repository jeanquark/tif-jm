                ...
                <div class="">
                    ...
                    <v-layout row wrap class="justify-center">
                        <v-flex xs12 sm6 class="text-xs-center">
                            <v-progress-linear height="20" :value="progress" color="primary"></v-progress-linear>
                            <span class="">{{ progress }}%</span>
                        </v-flex>

                        <v-flex xs12 class="my-3 text-xs-center">
                            <v-btn class="success" @click.stop="saveImage">Save</v-btn>
                            <v-btn class="error" nuxt to="/gamemode">Cancel</v-btn>
                        </v-flex>
                    </v-layout>
                </div>
            </v-flex>
...
<script>
	...
	export default {
		...
		methods: {
			...
			addToMerge(payload) {
				// console.log('Call to addMergeFromChild method in parent: ', payload)
			...
			mergeImages() {
				...
				if (process.browser) {
					mergeImages(this.cleanedArrayOfImagesToMerge)
						.then(b64 => (this.$refs.mergedImage.src = b64))
						.then
			...
			async saveImage() {
				try {
					...
					const now = moment.unix().utc()

					// 1) Try deleting old avatar if it exists, but do not fail if it doesn't work
					if (this.loadedUser.avatar) {
						try {
							const oldImageRef = firebase.storage().ref(`/images/avatars/${this.loadedUser.avatar.name}`)
					...
					uploadTask.on(
						...
						() => {
							...
							return this.$store
								.dispatch('users/updateAvatarImage', uploadTask)
								.then(() => {
					...
					console.log('error: ', error)
					new Noty({ type: 'error', text: 'Sorry, an error occured and your avatar could not be updated.', timeout: 5000, theme: 'metroui' }).show()
				}
			}
		}
	}
