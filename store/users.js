    ...
    async updateUser({ commit }, payload) {
        try {
            ...
            firebase
                ...
                .then(response => {
                    new Noty({
                        type: "success",
                        text:
                            "Modifications de l'utilisateur effectuées avec succès",
                        timeout: 5000,
                        theme: "metroui"
                    }).show()
                })
            ...
            new Noty({
                type: "error",
                text:
                    "Modifications de l'utilisateur non effectuées. Erreur: " +
                    error,
                timeout: 5000,
                theme: "metroui"
            }).show()
    ...
    async updateUserTeams2({ commit, state, dispatch }, payload) {
        try {
            ...
            const addedTeams = newTeams.filter(
                o => !oldTeams.find(o2 => o.slug === o2.slug)
            )

            // List dropped teams
            const droppedTeams = oldTeams.filter(
                o => !newTeams.find(o2 => o.slug === o2.slug)
            )
            ...
            let customText = ""
            if (teams_diff > 0) {
                customText = `You are following ${teams_diff} more ${
                    teams_diff > 1 ? "teams" : "team"
                } than before. This will cost you ${Math.abs(
                    total_tokens_cost
                )} tokens.`
            } else if (teams_diff < 0) {
                customText = `You are following ${Math.abs(teams_diff)} less ${
                    Math.abs(teams_diff) > 1 ? "teams" : "team"
                } than you used to. You will therefore gain ${Math.abs(
                    total_tokens_cost
                )} tokens.`
            } else {
                customText =
                    "You are following the same number of teams as before, so there is no cost for you."
            }
                    ...
                    firebase
                        ...
                        .then(() => {
                            ...
                            addedTeams.forEach(team => {
                                ...
                                new Noty({
                                    type: "success",
                                    text:
                                        "You are now also following " +
                                        team.name +
                                        ". Future will tell whether it's a wise move or not &#9786;",
                                    timeout: 5000,
                                    theme: "metroui"
                                }).show()
                            })
                        ...
                        .then(() => {
                            ...
                            return dispatch("updateUserTokens", {
                                ...
                                response => {
                                    console.log(
                                        "response from vuex updateUserTeams action: ",
                                        response
                                    )
    ...
    async updateUserEvents({ commit, getters }, payload) {
        ...
        try {
            ...
            new Noty({
                type: "error",
                text:
                    "Sorry, an error occured and the user events could not be updated",
                timeout: 5000,
                theme: "metroui"
            }).show()
            console.log(error)
